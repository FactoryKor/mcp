...
# MCP Python SDK v2(2026-07-28)에서 FastMCP가 MCPServer로 이름/모듈 경로가 바뀌었다
# (mcp.server.fastmcp 삭제). 고객 환경의 SDK 버전에 상관없이 기동되도록 분기한다.
try:
    from mcp.server.fastmcp import FastMCP as _ServerClass  # mcp 1.x
    _MCP_V2 = False
except ModuleNotFoundError:
    from mcp.server.mcpserver import MCPServer as _ServerClass  # mcp 2.x
    _MCP_V2 = True
...
# v2는 host/port를 생성자가 아니라 run()에서 받는다.
mcp = _ServerClass("diag-tools") if _MCP_V2 else _ServerClass("diag-tools", host=HOST, port=PORT)
...
if __name__ == "__main__":
    if _MCP_V2:
        mcp.run(transport="streamable-http", host=HOST, port=PORT)   # 엔드포인트: /mcp
    else:
